# -*- coding: utf-8 -*-
"""
Znajdowanie drogi wyjścia z labiryntu — ROZWIĄZANIE dla nauczyciela.
Klasa 3, informatyka w zakresie rozszerzonym, PCEiKZ Szczucin.

To jest wypełniony szkielet z docs/pliki/labirynt-szkielet.py — wszystkie
funkcje mają gotowe treści, wszystkie 10 testów przechodzi. NIE publikować
razem z materiałem; służy do sprawdzania prac i do pokazania rozwiązania
po oddaniu zadania.
...
def szukaj_rekurencyjnie(lab, start=None, wyjscie=None):
    ...
    def idz(pole):
        if pole in odwiedzone:
            return False
        odwiedzone.add(pole)
        droga.append(pole)
        if pole == wyjscie:
            return True
        for s2 in sasiedzi(lab, pole):
            if idz(s2):
                return True
        droga.pop()
        return False
...
def licz_nawroty(lab, start=None, wyjscie=None):
    ...
    start = start or znajdz(lab, "S")
    wyjscie = wyjscie or znajdz(lab, "E")
    odwiedzone, droga, licznik = set(), [], [0]
    ...
    return (droga if idz(start) else None, licznik[0])
...
def szukaj_iteracyjnie(lab, start=None, wyjscie=None):
    ...
    while stos:
        pole, i = stos[-1]
        if pole == wyjscie:
            return [p for p, _ in stos]
        if i >= len(KIERUNKI):
            stos.pop()
            continue
        stos[-1][1] += 1
        dw, dk = KIERUNKI[i]
        s2 = (pole[0] + dw, pole[1] + dk)
        if lab[s2[0]][s2[1]] != "#" and s2 not in odwiedzone:
            odwiedzone.add(s2)
            stos.append([s2, 0])
    return None
